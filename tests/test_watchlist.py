...
tests/test_collection.py so the two suites stay consistent.
"""
...
# ── Nonexistent film (Comment 3 — the requested test) ────────────────────────
...
# ── Deduplication (extra test — covers the Comment 2 dedup path) ──────────────

def test_add_to_watchlist_duplicate_raises(app, sample_user, sample_film):
    """
    Adding the same film to a watchlist twice should raise
    AlreadyInWatchlistError and must not create a second entry.
    """
    with app.app_context():
